...
def step_impl(context):
    ...
    if '/accounts/login' in current or 'login' in current:
        return
    denied_keywords = ['denegado', 'acceso denegado',
                       'no tienes permiso', 'not authorized', 'forbidden', '403']
    assert any(
...
def step_impl(context):
    ...
    try:
        table = context.driver.find_element(
            By.XPATH, "//h3[normalize-space()='Solicitudes por tipo']/following-sibling::table[1]")
...
def step_promedio_resolution(context, expected):
    ...
    m = re.search(
        r'Promedio de tiempo de resolución\s*[:]?[\s]*([\d:\sapmAPM]+)', body)
...
def step_responsable_count(context, name, count):
    ...
    try:
        table = context.driver.find_element(
            By.XPATH, "//h3[normalize-space()='Solicitudes por responsable']/following-sibling::table[1]")
    except Exception:
