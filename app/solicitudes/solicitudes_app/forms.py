...
class RegistroUsuarioForm(UserCreationForm):
    ...
    def clean_matricula(self):
        ...
        if not re.match(r'^\d{5,8}$', matricula):
            raise forms.ValidationError("La matrícula debe contener entre 5 y 8 dígitos.")
...
class ActualizarPerfilForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        if email:
            ...
            usuarios_con_email = Usuario.objects.filter(email=email).exclude(id=self.instance.id)
    ...
    def clean_matricula(self):
        ...
        if matricula:
            ...
            if not re.match(r'^\d{5,8}$', matricula):
                raise ValidationError('La matrícula debe contener entre 5 y 8 dígitos.')
            
            # Verificar que sea única (excepto el usuario actual)
            usuarios_con_matricula = Usuario.objects.filter(matricula=matricula).exclude(id=self.instance.id)
...
class GestionarUsuarioForm(forms.ModelForm):
    ...
    def clean_matricula(self):
        ...
        if matricula:
            ...
            if not re.match(r'^\d{5,8}$', matricula):
                raise ValidationError('La matrícula debe tener entre 5 y 8 dígitos.')
