...
def generar_pdf_graficas(request):
    ...
    def crear_grafico(data, titulo):
        ...
        colores_barras = ['#c9a24d', '#202146',
                          '#d7d7d7', '#c9a24d', '#202146']
        bars = ax.bar(tipos, totales,
                      color=colores_barras[:len(tipos)], width=0.6)
    ...
    fecha_generacion = Paragraph(
        f"Reporte generado el {hoy.strftime('%d/%m/%Y')}", styles['Normal'])
    elements.append(Spacer(1, 0.3*inch))
...
def crear_solicitud_usuario(request):
    ...
    if not _procesar_get_request(request, tipo_id):
        return redirect('crear_solicitud_usuario')
    
    formulario_solicitud, campos, tipo = _obtener_formulario_y_campos(tipo_id)
    
    if request.method == 'POST':
        return _procesar_post_request(request, tipo_id, formulario_solicitud, campos, tipo)
    
    return _renderizar_contexto(request, tipos_disponibles, formulario_solicitud, campos, tipo_id)

# Funciones auxiliares
def _procesar_get_request(request, tipo_id):
    """Procesa la petición GET y valida el tipo de solicitud"""
    if tipo_id:
        try:
            tipo = get_object_or_404(TipoSolicitud, id=tipo_id)
            FormularioSolicitud.objects.get(tipo_solicitud=tipo)
            return True
...
def _obtener_formulario_y_campos(tipo_id):
    """Obtiene el formulario y campos asociados al tipo de solicitud"""
    if not tipo_id:
        return None, [], None
    ...
    return formulario_solicitud, campos, tipo

def _validar_campos(request, campos):
    """Valida todos los campos del formulario"""
    errores = []
    archivos_validados = {}
    
    for campo in campos:
        _validar_campo_individual(request, campo, errores, archivos_validados)
    
    return errores, archivos_validados

def _validar_campo_individual(request, campo, errores, archivos_validados):
    """Valida un campo individual del formulario"""
    if campo.tipo == 'file':
        _validar_campo_archivo(request, campo, errores, archivos_validados)
    else:
        _validar_campo_textual(request, campo, errores)

def _validar_campo_archivo(request, campo, errores, archivos_validados):
    """Valida campos de tipo archivo"""
    archivos = request.FILES.getlist(f'campo_{campo.id}')
    
    if campo.requerido and not archivos:
    ...
    if archivos:
        ...
        for archivo in archivos:
            try:
                ...
                errores.append(f'{campo.etiqueta}: {str(e)}')
        
        if archivos_validos:
            archivos_validados[campo.id] = archivos_validos

def _validar_campo_textual(request, campo, errores):
    """Valida campos textuales"""
    if campo.requerido:
        valor = request.POST.get(f'campo_{campo.id}', '').strip()
        if not valor:
            errores.append(f'El campo "{campo.etiqueta}" es obligatorio')

def _crear_solicitud_y_respuestas(request, tipo, campos, archivos_validados):
    """Crea la solicitud y sus respuestas"""
    ...
    _guardar_respuestas(solicitud, campos, request, archivos_validados)
    _crear_seguimiento_inicial(solicitud)
    
    return folio

def _guardar_respuestas(solicitud, campos, request, archivos_validados):
    """Guarda las respuestas de la solicitud"""
    for campo in campos:
        if campo.tipo == 'file' and campo.id in archivos_validados:
            _guardar_respuesta_archivo(solicitud, campo, archivos_validados[campo.id])
        else:
            _guardar_respuesta_textual(solicitud, campo, request)

def _guardar_respuesta_archivo(solicitud, campo, archivos):
    """Guarda respuestas de tipo archivo"""
...
def _guardar_respuesta_textual(solicitud, campo, request):
    """Guarda respuestas de tipo textual"""
...
def _crear_seguimiento_inicial(solicitud):
    """Crea el seguimiento inicial de la solicitud"""
...
def _procesar_post_request(request, tipo_id, formulario_solicitud, campos, tipo):
    """Procesa la petición POST"""
    if not tipo_id:
        messages.error(request, 'No se ha seleccionado un tipo de solicitud')
        return redirect('crear_solicitud_usuario')
    
    errores, archivos_validados = _validar_campos(request, campos)
    
    if errores:
        for error in errores:
            messages.error(request, error)
    else:
        folio = _crear_solicitud_y_respuestas(request, tipo, campos, archivos_validados)
        messages.success(request, f'Solicitud creada exitosamente. Folio: {folio}')
        return redirect('mis_solicitudes')
    
    return None

def _renderizar_contexto(request, tipos_disponibles, formulario_solicitud, campos, tipo_id):
    """Renderiza el contexto para la plantilla"""
    context = {
...
def mis_solicitudes(request):
    ...
    # Subconsulta para obtener el último seguimiento
    ultimo_seguimiento = SeguimientoSolicitud.objects.filter(
        solicitud=OuterRef('pk')
    ).order_by('-fecha_creacion')
